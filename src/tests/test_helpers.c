...
START_TEST(s21_is_zero_fourth_bit_is_digit){
    s21_big_decimal val = {{0, 0, 0, 40, 0, 0, 0, 0}};
...
START_TEST(s21_is_zero_multiple_nonzero) {
    s21_big_decimal val = {{1, 2, 0, 0, 0, 0, 0, 0}};
    ck_assert_int_eq(s21_is_zero(val), 0);
}
END_TEST

START_TEST(s21_is_zero_last_bit_nonzero) {
    s21_big_decimal val = {{0, 0, 0, 0, 0, 0, 0, 5}};
    ck_assert_int_eq(s21_is_zero(val), 1);
}
END_TEST
...
Suite* s21_helpers_suite(void) {
    ...
    tcase_add_test(tc_core, s21_is_zero_last_bit_nonzero);
