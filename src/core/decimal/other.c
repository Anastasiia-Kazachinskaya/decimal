...
static int s21_compute_fractional_big(s21_decimal original,
...
int s21_round(s21_decimal value, s21_decimal* result) {
  ...
  if (s21_compute_fractional_big(value, *result, &fractional) != OK)
    return CALCULATION_ERROR;

  if (s21_apply_rounding(result, &fractional, &half) != OK)
    return CALCULATION_ERROR;
...
static int s21_increment_mantissa(s21_decimal* value) {
  ...
  for (int i = 0; i < 3; i++) {
    if (++value->bits[i] !=
        0) {  // при переполнении bit[i] = 0, bits[1] = 1 etc.
      status = OK;
      break;
    }
  }
...
static void s21_pow10_big(int scale, s21_big_decimal* result) {
  ...
  for (int i = 0; i < scale; i++) {
    uint64_t carry = 0;
    for (int j = 0; j < 7; j++) {
      uint64_t prod = (uint64_t)result->bits[j] * 10ULL +
                      carry;  // 10ULL (unsigned long long) 64 битное умножение
...
static void s21_big_div2(s21_big_decimal* value) {
  ...
  for (int i = 6; i >= 0; i--) {
    ...
    value->bits[i] =
        (current >> 1) |
        (remainder ? 0x80000000u : 0);  // 0x80000000u - 32-битное число с
                                        // установленным старшим битом = 2^31
...
static int s21_compute_fractional_big(s21_decimal original,
                                      ...
                                      s21_big_decimal* fractional) {
  int status = OK;
  s21_big_decimal big_orig =
      s21_decimal_to_big_internal(&original);  // исходное число
  s21_big_decimal big_truncated =
      s21_decimal_to_big_internal(&truncated);  // целая часть
